...
typedef struct BKAudioUserData {
    UInt32 mChannels;
    UInt32 mDataSize;
    void *mData;
...
@implementation BKAudioDecoder

- (instancetype)init {
    self = [super init];
    if (self) {
        _decoderQueue = dispatch_queue_create("AudioDecoder", DISPATCH_QUEUE_SERIAL);
    }
...
- (void)_decodeSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    if (outAudioBufferList.mBuffers[0].mDataByteSize > 0) {
        ...
        // 11、回调解码数据。
        if (pcmSampleBuffer) {
            if (self.delegate && [self.delegate respondsToSelector:@selector(audioDecodeSuccessCallback:)]) {
                [self.delegate audioDecodeSuccessCallback:pcmSampleBuffer];
            }
...
- (void)_callBackError:(NSError*)error {
    self.isError = YES;
    if (error && self.delegate && [self.delegate respondsToSelector:@selector(audioDecodeErrorCallback:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.delegate audioDecodeErrorCallback:error];
        });
    }
}
...
static OSStatus inputDataProcess(AudioConverterRef inConverter, UInt32 *ioNumberDataPackets, AudioBufferList *ioData, AudioStreamPacketDescription **outDataPacketDescription, void *inUserData) {
    BKAudioUserData *userData = (BKAudioUserData *) inUserData;
