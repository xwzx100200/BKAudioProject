...
@property (nonatomic, strong, readwrite) BKAudioConfig *config;
@property (nonatomic, strong) dispatch_queue_t captureQueue;
@property (nonatomic, assign) BOOL isError;
@end

@implementation BKAudioCapture
...
- (instancetype)initWithConfig:(BKAudioConfig *)config {
    ...
    if (self) {
        _config = config;
        _captureQueue = dispatch_queue_create("audioCapture", DISPATCH_QUEUE_SERIAL);
    }
...
- (void)startRunning {
    ...
    dispatch_async(_captureQueue, ^{
        ...
        if (startStatus != noErr) {
            // 捕捉并回调开始采集时的错误。
            [weakSelf callBackError:[NSError errorWithDomain:NSStringFromClass([BKAudioConfig class]) code:startStatus userInfo:nil]];
        }
    });
}
...
- (void)stopRunning {
    ...
    dispatch_async(_captureQueue, ^{
        if (weakSelf.audioCaptureInstance) {
            ...
            if (stopStatus != noErr) {
                // 捕捉并回调停止采集时的错误。
                [weakSelf callBackError:[NSError errorWithDomain:NSStringFromClass([BKAudioConfig class]) code:stopStatus userInfo:nil]];
            }
        }
    });
}
...
- (void)setupAudioCaptureInstance:(NSError **)error {
    ...
    asbd.mChannelsPerFrame = (UInt32) self.config.channelCount; // 每帧的声道数
    asbd.mFramesPerPacket = 1; // 每个数据包帧数
    asbd.mBitsPerChannel = (UInt32) self.config.sampleSize; // 采样位深
...
static OSStatus audioBufferCallBack(void *inRefCon,
    ...
    @autoreleasepool {
        BKAudioCapture *capture = (__bridge BKAudioCapture *) inRefCon;
        if (!capture) {
            return -1;
        }
        ...
        if (status == noErr) {
            ...
            CMSampleBufferRef sampleBuffer = [BKAudioCapture sampleBufferFromAudioBufferList:buffers inTimeStamp:inTimeStamp inNumberFrames:inNumberFrames description:capture.audioFormat];
            // 回调数据。
