...
@implementation BKAudioEncoder

#pragma mark - Lifecycle
- (instancetype)initWithConfig:(BKAudioConfig *)config {
    self = [super init];
    if (self) {
        _audioBitrate = config.bitrate;
...
- (void)encodeBuffer:(char *)buffer timing:(CMSampleTimingInfo)timing {
    ...
    // 5、回调编码数据。
    if (self.delegate && [self.delegate respondsToSelector:@selector(audioEncodeSuccessCallback:)]) {
        [self.delegate audioEncodeSuccessCallback:sampleBuffer];
    }
    if (sampleBuffer) {
...
- (void)callBackError:(NSError *)error {
    self.isError = YES;
    if (error && self.delegate && [self.delegate respondsToSelector:@selector(audioEncodeErrorCallback:)]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.delegate audioEncodeErrorCallback:error];
        });
    }
}
